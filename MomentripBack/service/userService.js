...
module.exports = {
    ...
    followUSer : async (following_id, followed_id, res) => {
        if(!following_id || !followed_id){
            console.log("필요값 누락");
            return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, responseMessage.FOLLOW_USER_FAIL));
        }
        try{
            const followingUser = await userMethod.findById(following_id);
            const followedUser = await userMethod.findById(followed_id);
            if(!followingUser || !followedUser) return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, responseMessage.FOLLOW_USER_FAIL));
            await userMethod.following(followingUser, followed_id);
            // await user.addFollowing(parseInt(req.params.id,10));
            return res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.FOLLOW_USER_SUCCESS, user));
        }catch (err){
            console.error(err);
